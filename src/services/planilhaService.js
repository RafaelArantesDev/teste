...
function montarTabelaHolerite(value) {
    ...
    for (const pagina of paginas) {
        ...
        const paginaVazia = (!Array.isArray(pagina?.fields) || pagina.fields.length === 0)
            && (!Array.isArray(pagina?.bases) || pagina.bases.length === 0);
        const possuiInterrogacao = [
            ...valores,
            ...(Array.isArray(pagina?.bases) ? pagina.bases.flatMap(base => [base?.label, base?.value]) : [])
        ].some(valor => String(valor ?? '').includes('?'));

        linhas.push({ valores, aviso: paginaVazia || possuiInterrogacao, naoSequencial });
    }
...
function montarTabelaCartaoPonto(value) {
    ...
    for (const dia of dias) {
        ...
        const naoSequencial = Boolean(dataAnterior && dataAtual && !dataSeguinte(dataAnterior, dataAtual));
...
function objetoDaLinha(colunas, linha) {
    const objeto = {};
    colunas.forEach((coluna, indice) => { objeto[coluna] = linha[indice] ?? ''; });
    return objeto;
}
...
function lerCompetencia(month, year) {
    ...
    return { mes: numeroMes, ano: Number(ano) };
}
...
function gerarXlsx(tabela) {
    const arquivos = [
        ...
        { nome: 'xl/styles.xml', conteudo: gerarStylesXml() },
        { nome: 'xl/worksheets/sheet1.xml', conteudo: gerarWorksheetXml(tabela) }
    ];
    return criarZip(arquivos);
}

function gerarStylesXml() {
    return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="11"/><name val="Calibri"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="11"/><name val="Calibri"/></font></fonts><fills count="5"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF173772"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFFF3CD"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFF8D7DA"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="2"><border><left/><right/><top/><bottom/><diagonal/></border><border><left style="medium"><color rgb="FFDC3545"/></left><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="5"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1"><alignment horizontal="center"/></xf><xf numFmtId="0" fontId="0" fillId="3" borderId="0" xfId="0" applyFill="1"/><xf numFmtId="0" fontId="0" fillId="4" borderId="0" xfId="0" applyFill="1"/><xf numFmtId="0" fontId="0" fillId="4" borderId="1" xfId="0" applyFill="1" applyBorder="1"/></cellXfs></styleSheet>`;
}

function gerarWorksheetXml(tabela) {
    const linhas = [{ valores: tabela.colunas, cabecalho: true, aviso: false, naoSequencial: false }, ...tabela.linhas];
    ...
    const rowsXml = linhas.map((linha, indiceLinha) => {
        ...
        const cells = linha.valores.map((valor, indiceColuna) => {
            ...
            const estilo = linha.cabecalho ? 1 : linha.naoSequencial ? (indiceColuna === 0 ? 4 : 3) : linha.aviso ? 2 : 0;
